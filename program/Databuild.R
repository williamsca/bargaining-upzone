...
# Import ----
# dt_elig <- readRDS("derived/County Eligibility (2000-2022).Rds")
dt_bp <- readRDS(
...
# Building Permits ----
dt_bp[Name == "Bedford (Independent City)", `:=`(Name = "Bedford County", FIPS.Code.County = "019")] # Bedford City became a town on July 1, 2013. https://en.wikipedia.org/wiki/Bedford,_Virginia
dt_bp[Name == "Clifton Forge (Independent Cit", `:=`(Name = "Alleghany County", FIPS.Code.County = "005")] # Clifton Forge became a town in 2001. https://en.wikipedia.org/wiki/Clifton_Forge,_Virginia

v.cols <- grep("(Bldgs|Value|Units)", names(dt_bp), value = TRUE)
v.cols <- grep("rep", v.cols, value =  TRUE, invert = TRUE)
dt_bp <- dt_bp[, lapply(.SD, sum), by = .(Year4, Month, FIPS.Code.State, FIPS.Code.County, Name), .SDcols = v.cols] # combine Clifton Forge and Bedford with the surrounding county.

dt_bp[, Name := gsub("\\s*\\(.*", " City", Name)]

dt_bp[, FY := Year4 + fifelse(Month >= 7, 1, 0)] # The VA fiscal year runs from July 1 to June 30

# Note: some proffer-collecting jurisdictions are not covered in the county building permits survey
dt <- merge(dt_bp, dt_cp[, FIPS.Code.State := "51"], by.x = c("Name", "FY", "FIPS.Code.State"), by.y = c("Jurisdiction", "Year", "FIPS.Code.State"), 
            all.x = TRUE)
...
dt <- merge(dt, dt_zhvi[, .(Date, FIPS, ZHVI, RegionName)], by = c("Date", "FIPS"), all.x = TRUE)
